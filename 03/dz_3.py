...
def sum_lists(list1, list2):
    ...
    if len(list1) < len(list2):
        list1.extend([0] * (len(list2)-len(list1))) #extend- вставити iterable
    elif len(list1) > len(list2):
        list2.extend([0] * (len(list1)-len(list2)))

    # Використовуємо генератор для повернення суми елементів списку, zip- об'єднати 2 в 1
...
cubed = calculate([1, 2, 3, 4], lambda x: x ** 3)   # [1, 8, 27, 64] - куби чисел
...
@report_decorator     # Декорування звітної функції
def employee_report(department):   # Звичайна функція звіту про співробітників за відділами
    print(f'Звіт про співробітників відділу {department}.')
